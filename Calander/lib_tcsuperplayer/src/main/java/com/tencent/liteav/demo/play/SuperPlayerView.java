...
public class SuperPlayerView extends RelativeLayout implements ITXVodPlayListener, ITXLivePlayListener {
    ...
    private TvIconListener tvIconListener;
    private OverTurnListener overTurnListener;
...
//        LogReport.getInstance().setPackageName(getPackagename());
    ...
    public void setTvIconListener(TvIconListener tvIconListener) {
        this.tvIconListener = tvIconListener;
        if (mVodControllerLarge != null) {
            mVodControllerLarge.setTvIconListener(this.tvIconListener);
        }
    }

    public void setOverTurnListener(OverTurnListener overTurnListener) {
        this.overTurnListener = overTurnListener;
    }
...
//            }
        ...
        @Override
        public void windowModel(int model) {
            if (mVodPlayer != null) {
                mVodPlayer.setRenderMode(model);
            }
        }

        @Override
        public void onOverTurn(boolean overTurn) {
            if (overTurnListener != null) {
                overTurnListener.overTurn(overTurn);
            }
        }
    };
